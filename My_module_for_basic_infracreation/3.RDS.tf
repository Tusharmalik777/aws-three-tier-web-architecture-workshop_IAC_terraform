...
resource "aws_rds_cluster" "database-server" {
  engine                 = "aurora-mysql"
  master_username        = "admin"
  master_password        = "Welcome$1041"
  cluster_identifier     = "database-server"
...
resource "aws_rds_cluster_instance" "writer_instance" {
  cluster_identifier  = aws_rds_cluster.database-server.id
...
resource "aws_rds_cluster_instance" "reader_instance" {
  ...
  instance_class      = "db.t3.medium"
  engine              = "aurora-mysql"
  identifier          = "reader-instance-${count.index}"
  publicly_accessible = false
}